...
CIHM::Meta::OcrPdf - Generate multi-page PDF files from OCR data associated with image canvases.
...
sub new {
    ...
    if ( ref($args) ne "HASH" ) {
        die "Argument to CIHM::Meta::OcrPdf->new() not a hash\n";
    }
    $self->{args} = $args;
...
sub OcrPdf {
    ...
    if ( $self->maxprocs == 1 ) {
        while ( my $noid = $self->getNextNOID() ) {
            $somework = 1;
            CIHM::Meta::OcrPdf::Worker::createpdf( $noid,
                encode_json $self->args );
        }
    }
    else {
        my $pool =
          AnyEvent::Fork->new->require("CIHM::Meta::OcrPdf::Worker")
          ->AnyEvent::Fork::Pool::run(
            "CIHM::Meta::OcrPdf::Worker::createpdf",
            max        => $self->maxprocs,
            load       => 2,
...
sub getNextNOID {
    ...
    if ( defined $self->{slug} ) {
        ...
        my $url =
            "/_design/metadatabus/_view/OCRPDFQueue?reduce=false&limit="
          . $self->limit
          . $skipparam;
        my $res =
